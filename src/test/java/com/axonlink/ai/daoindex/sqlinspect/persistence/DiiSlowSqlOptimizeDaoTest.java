...
@DisplayName("DiiSlowSqlOptimizeDao —— 真身表 CRUD + 未生效标记 + 工号/姓名/优化内容")
class DiiSlowSqlOptimizeDaoTest {
    ...
    void setUp() {
        ...
        jdbc.execute("CREATE TABLE dii_slow_sql_optimization ("
                ...
                + "optimized_by_name VARCHAR(64),"
                + "optimize_note VARCHAR(200),"
    ...
    @Test @DisplayName("upsert 首次插入 OPTIMIZED，记工号/姓名/优化内容")
    void upsertInsertsOptimized() {
        dao.upsertOptimized("svcA", "h1", "20260601-1", "1001", "张三", "给 nxt_bal 加联合索引", LocalDateTime.now());
        ...
        assertEquals("给 nxt_bal 加联合索引", rec.get("optimize_note"));
    }

    @Test @DisplayName("upsert 重复键：刷新 R0/工号/姓名/内容 并清 reappeared，仍只有一行")
    void upsertOnDuplicateRefreshes() {
        dao.upsertOptimized("svcA", "h1", "20260601-1", "1001", "张三", "加索引", LocalDateTime.now());
        dao.flagReappeared("svcA", "h1", "20260701-1", LocalDateTime.now());
        dao.upsertOptimized("svcA", "h1", "20260701-1", "1002", "李四", "改写 SQL 去掉全表扫描", LocalDateTime.now());
        Map<String, Object> rec = dao.findByKey("svcA", "h1");
        assertEquals("OPTIMIZED", rec.get("status"));
        assertEquals("20260701-1", rec.get("optimized_round"));
        assertNull(rec.get("reappeared_round"));
        assertEquals("李四", rec.get("optimized_by_name"));
        assertEquals("改写 SQL 去掉全表扫描", rec.get("optimize_note"));
    ...
    void listAllReturnsAll() {
        dao.upsertOptimized("svcA", "h1", "20260601-1", "1001", "张三", "加索引", LocalDateTime.now());
        dao.upsertOptimized("svcB", "h2", "20260601-1", "1001", "张三", "加索引", LocalDateTime.now());
