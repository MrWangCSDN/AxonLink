...
CREATE TABLE dii_error_code (
    ...
    throw_text   MEDIUMTEXT    NOT NULL COMMENT 'throw 后的完整内容(AST打印)；银行嵌套表达式可能很长，故 MEDIUMTEXT',
    ...
    file_path    VARCHAR(2048)          COMMENT '源文件绝对路径（内网路径较深，放宽到 2048）',
...
CREATE TABLE dii_tx_error_code (
    ...
    throw_text    MEDIUMTEXT    NOT NULL COMMENT 'throw 完整内容；同 dii_error_code，MEDIUMTEXT',
    class_fqn     VARCHAR(255)  NOT NULL COMMENT '源类 FQN',
    method_name   VARCHAR(128)  NOT NULL COMMENT '源方法简名',
    file_path     VARCHAR(2048)          COMMENT '源文件路径（放宽到 2048）',
